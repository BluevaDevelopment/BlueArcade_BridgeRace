package net.blueva.arcade.modules.bridge_race;
...
import net.blueva.arcade.modules.bridge_race.listener.BridgeRaceListener;
import net.blueva.arcade.modules.bridge_race.setup.BridgeRaceSetup;
...
public class BridgeRaceModule implements GameModule<Player, Location, World, Material, ItemStack, Sound, Block, Entity, Listener, EventPriority> {

    private ModuleConfigAPI moduleConfig;
    private CoreConfigAPI coreConfig;
    private ModuleInfo moduleInfo;
    private BridgeRaceGameManager gameManager;
    private BridgeRaceStatsService statsService;

    @Override
    public void onLoad() {
        moduleInfo = ModuleAPI.getModuleInfo("bridge_race");

        if (moduleInfo == null) {
            throw new IllegalStateException("ModuleInfo not available for bridge_race module");
        }
        ...
        statsService = new BridgeRaceStatsService(statsAPI, moduleInfo);
        ...
        ModuleAPI.getSetupAPI().registerHandler(moduleInfo.getId(), new BridgeRaceSetup(moduleConfig, coreConfig));
        ...
        gameManager = new BridgeRaceGameManager(moduleInfo, moduleConfig, coreConfig, statsService);
    }
    ...
    public void registerEvents(CustomEventRegistry<Listener, EventPriority> registry) {
        registry.register(new BridgeRaceListener(gameManager, gameManager.getStateRegistry(), gameManager.getStatsService()));
    }
    ...
    public ModuleSetupMetadata getSetupMetadata() {
        return new ModuleSetupMetadata() {
            ...
            public List<ModuleSetupStep> getSetupSteps() {
                return List.of(
                        new ModuleSetupStep("finishline", true, "Configure Finishline", "Configure the module-specific finishline setup data.", List.of("/baa game <arena> bridge_race finishline"), "selection region")
                );
            }
            ...
            public List<ModuleSetupCommand> getSetupCommands() {
                return List.of(
                        new ModuleSetupCommand("finishline", "/baa game <arena> bridge_race finishline", "Configure finishline setup data.", true)
                );
            }
