package net.blueva.arcade.modules.bridge_race.game;
...
import net.blueva.arcade.modules.bridge_race.support.BridgeRaceLoadoutService;
import net.blueva.arcade.modules.bridge_race.support.BridgeRaceMessagingService;
import net.blueva.arcade.modules.bridge_race.support.BridgeRaceProgressService;
...
public class BridgeRaceGameManager {

    private final ModuleInfo moduleInfo;
    private final ModuleConfigAPI moduleConfig;
    private final CoreConfigAPI coreConfig;
    private final BridgeRaceStatsService statsService;
    private final BridgeRaceLoadoutService loadoutService;
    private final BridgeRaceMessagingService messagingService;
    private final BridgeRaceProgressService progressService;
    private final BridgeRaceStateRegistry stateRegistry;

    public BridgeRaceGameManager(ModuleInfo moduleInfo,
                           ModuleConfigAPI moduleConfig,
                           CoreConfigAPI coreConfig,
                           BridgeRaceStatsService statsService) {
        this.moduleInfo = moduleInfo;
        this.moduleConfig = moduleConfig;
        this.coreConfig = coreConfig;
        this.statsService = statsService;
        this.stateRegistry = new BridgeRaceStateRegistry();
        this.loadoutService = new BridgeRaceLoadoutService(moduleConfig);
        this.progressService = new BridgeRaceProgressService();
        this.messagingService = new BridgeRaceMessagingService(moduleInfo, moduleConfig, coreConfig, progressService);
    }

    public BridgeRaceStateRegistry getStateRegistry() {
        return stateRegistry;
    }

    public BridgeRaceStatsService getStatsService() {
        return statsService;
    }
    ...
    private void startGameTimer(GameContext<Player, Location, World, Material, ItemStack, Sound, Block, Entity> context) {
        ...
        String taskId = "arena_" + arenaId + "_bridge_race_timer";
    ...
    public void handlePlayerRespawn(Player player) {
        loadoutService.prepareForRespawn(player);
    }
    ...
    public void handleEnd(GameContext<Player, Location, World, Material, ItemStack, Sound, Block, Entity> context) {
        ...
        // Clear placed blocks before arena state is cleared
        BridgeRaceArenaState state = stateRegistry.getState(arenaId);
        if (state != null) {
            state.clearPlacedBlocks();
        }
    ...
    public Map<String, String> getCustomPlaceholders(Player player) {
        ...
        if (context != null) {
            ...
            placeholders.put("bridge_race_position", String.valueOf(position));
